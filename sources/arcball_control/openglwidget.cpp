...
: QOpenGLWidget(parent) {
    arcball = std::make_unique<ArcballCamera>(this);
...
void OpenGLWidget::initializeGL() {
    ...
    arcball->setLookAt(QVector3D(3.0f, 4.0f, 5.0f), QVector3D(0.0f, 0.0f, 0.0f), QVector3D(0.0f, 1.0f, 0.0f));
    arcball->setPerspective(45.0f, (float)width() / (float)height(), 0.1f, 1000.0f);
}
...
void OpenGLWidget::paintGL() {
    ...
    shader->setUniformValue("u_mvpMat", arcball->mvpMat());
    shader->setUniformValue("u_mvMat", arcball->mvMat());
    shader->setUniformValue("u_viewMat", arcball->viewMat());
    shader->setUniformValue("u_normMat", arcball->normMat());
